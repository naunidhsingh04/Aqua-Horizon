"""
AQUA HORIZON — Model 3: CNN + Transformer
...
from ml.models_hybrid import CNNTransformer
from ml.data_loader import load_sequence_data

def train_cnn___transformer():
    print("=" * 80)
    print("AQUA HORIZON — HYBRID MODEL 3: CNN + TRANSFORMER")
    print("Architecture Class: CNNTransformer")
    print("=" * 80)
    ...
    data = load_sequence_data(seq_len=5, cutoff_year=2012, spatial=False, batch_size=128)
    ...
    best_f1 = 0.0
    best_metrics = None
    save_path = os.path.join(os.path.dirname(__file__), 'models', 'cnn_transformer.pt')
    
    print(f"\nBeginning Training (10 Epochs)...")
    ...
    for epoch in range(1, 10 + 1):
        ...
        print(f"  Epoch [{epoch:02d}/10] - Loss: {epoch_loss:.4f} | Test F1: {best_t_f1:.4f} | Recall: {best_t_rec*100:.2f}% | Prec: {best_t_prec*100:.2f}% | ROC-AUC: {roc_auc:.4f} (th={best_t:.2f})")
    ...
    # Save metrics JSON
    metrics_path = os.path.join(os.path.dirname(__file__), 'models', 'cnn_transformer_metrics.json')
    with open(metrics_path, 'w') as f:
        json.dump(best_metrics, f, indent=2)
        
    print("-" * 80)
    print("TRAINING COMPLETE FOR CNN + TRANSFORMER!")
