"""
AQUA HORIZON — Model 1: U-Net + ConvLSTM
...
from ml.models_hybrid import UNetConvLSTM
from ml.data_loader import load_sequence_data

def train_u_net___convlstm():
    print("=" * 80)
    print("AQUA HORIZON — HYBRID MODEL 1: U-NET + CONVLSTM")
    print("Architecture Class: UNetConvLSTM")
    print("=" * 80)
    ...
    best_f1 = 0.0
    best_metrics = None
    save_path = os.path.join(os.path.dirname(__file__), 'models', 'unet_convlstm.pt')
    ...
    # Save metrics JSON
    metrics_path = os.path.join(os.path.dirname(__file__), 'models', 'unet_convlstm_metrics.json')
    with open(metrics_path, 'w') as f:
        json.dump(best_metrics, f, indent=2)
        
    print("-" * 80)
    print("TRAINING COMPLETE FOR U-NET + CONVLSTM!")
