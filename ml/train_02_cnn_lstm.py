...
from ml.models_hybrid import CNNLSTM
from ml.data_loader import load_sequence_data

def train_cnn___lstm():
    print("=" * 80)
    print("AQUA HORIZON — HYBRID MODEL 2: CNN + LSTM")
    print("Architecture Class: CNNLSTM")
    print("=" * 80)
    ...
    best_f1 = 0.0
    best_metrics = None
    save_path = os.path.join(os.path.dirname(__file__), 'models', 'cnn_lstm.pt')
    ...
    # Save metrics JSON
    metrics_path = os.path.join(os.path.dirname(__file__), 'models', 'cnn_lstm_metrics.json')
    with open(metrics_path, 'w') as f:
        json.dump(best_metrics, f, indent=2)
        
    print("-" * 80)
    print("TRAINING COMPLETE FOR CNN + LSTM!")
